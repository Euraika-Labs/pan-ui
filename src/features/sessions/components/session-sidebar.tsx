...
export function SessionSidebar({ sessions, selectedSessionId, search, isLoading, onSearchChange, onNewChat, onSelectSession }: SessionSidebarProps) {
  ...
  return (
    <aside className="flex h-full max-h-full flex-col overflow-hidden rounded-[1.9rem] border border-border/70 bg-card/72 shadow-[var(--shadow-soft)]">
      <div className="border-b border-border/70 p-4">
        <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Workspace</p>
        <div className="rounded-[1.45rem] bg-[linear-gradient(135deg,hsl(var(--primary))/0.16,hsl(var(--accent))/0.12)] p-1">
          <button
            type="button"
            onClick={onNewChat}
            className="flex w-full items-center justify-center gap-2 rounded-[1.15rem] bg-[linear-gradient(135deg,hsl(var(--primary)),hsl(var(--accent)))] px-4 py-3 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-card)] transition hover:-translate-y-0.5"
          >
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
          <div className="rounded-2xl border border-border/70 bg-background/70 px-2 py-2.5">
            <p className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">All</p>
            <p className="mt-1 text-sm font-semibold text-foreground">{visibleSessions.length}</p>
          </div>
          <div className="rounded-2xl border border-border/70 bg-background/70 px-2 py-2.5">
            <p className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">Pinned</p>
            <p className="mt-1 text-sm font-semibold text-foreground">{pinnedSessions.length}</p>
          </div>
          <div className="rounded-2xl border border-border/70 bg-background/70 px-2 py-2.5">
            <p className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">Archived</p>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
        {isLoading ? <div className="rounded-2xl border border-dashed border-border/70 bg-background/70 p-4 text-sm text-muted-foreground">Loading sessions…</div> : null}
        {!isLoading && visibleSessions.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/70 bg-background/70 p-4 text-sm text-muted-foreground">No sessions match your search.</div>
        ) : null}
        {groups.map((group) => (
          <div key={group.label} className="space-y-2">
            <div className="flex items-center justify-between px-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedSessionId === session.id
                    ? 'border-primary/20 bg-primary/10 shadow-[var(--shadow-card)]'
                    : 'border-border/70 bg-background/75 hover:border-border hover:bg-card',
                )}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    {session.pinned ? (
                      <span className="inline-flex items-center gap-1 rounded-full border border-border/70 px-2 py-0.5 text-[10px] uppercase tracking-[0.15em] text-muted-foreground">
